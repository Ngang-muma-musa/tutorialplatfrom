...
if (isset($_POST["register"])) {
	...
	if (!filter_var($email,FILTER_VALIDATE_EMAIL)&&!preg_match("/[a-zA-Z0-9]/",$firstName)&&!preg_match("/[a-zA-Z0-9]/",$lastName)) {
		header("Location:../register.php? error=Invalid Fields");
	    exit();
	}
	...
	elseif ($pwd !== $pwd2) {
        ...
        if (!mysqli_stmt_prepare($stmt,$sql)) {
		    ...
		    if ($resultcheck > 0) {
		    	...
		    	$sql2 = "INSERT INTO teachers (firstName,lastName,email,password) VALUES (?,?,?,?)";
			    ...
			    if (!mysqli_stmt_prepare($stmt2,$sql2)) {
					...
					header("Location:../teachers.php?signup=SignUp Successful");
		            exit();
				}
		    }    
	    }
